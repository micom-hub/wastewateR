...
#'
#' This function does not return anything, it is just a "check" function. This
...
#' @param stop_q A "yes" or "no" character string, indicating whether the limit should be a hard stop limit (yes), or just a warning (no).
#' @return Nothing
#' @export

w0650_cumulative_count_check <- function(df_file_in, set_limit = 3, stop_q = "no"){

  if (!trimws(tolower(stop_q)) %in% c("yes", "no")){
