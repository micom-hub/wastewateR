...
#' Wrapper Function - QAQC for PMMoV and BCoV File
#'
...
qaqc_processing_pmmov_bcov <- function(file_in,
                                   lab_site_ids,
                                   targets_to_keep = c("BCOV", "PMMOV"),
                                   control_strings = c("NEG", "NTC", "BCOV", "EXT"),
                                   expected_count = c(3, 3, 3, 3),
                                   pos_rows = data.frame(Samples = c("BCOV"),
                                                         Targets = c("BCOV")),
                                   con_rows = data.frame(Samples = c("BCOV", "BCOV", "EXT", "EXT","NEG","NEG", "NTC",  "NTC"),
                                                         Targets = c("PMMOV", "BCOV", "PMMOV",  "BCOV", "PMMOV",  "BCOV", "PMMOV",  "BCOV")),
                                   should_be_neg = data.frame(Samples = c("NEG", "EXT"),
                                                              Targets = c("PMMOV", "PMMOV")),
                                   recover_unit = "BCOV",
                                   rules = c(2, 3, 4, 5.5, 6),
