...
#' 
#' This function returns a dataframe with a new column called bcov_flag that 
...
#' @return A dataframe just like the input data frame, with one new column (bcov_flag) added
#' @export
...
w0800_recover_control_check <- function(new_file_in, recovery_control, control_ids, limit_percent = 0.3){
  ...
  if (nrow(bcov_targets) > 0){
    ...
    if (nrow(bcov_sample_bcov_target) > 0){
      ...
      bcov_targets <- bcov_targets %>% mutate(bcov_flag = case_when(average_positives < bcov_30_limit ~ 1, 
                                                                    T ~ 0))
      
      if (any(bcov_targets$bcov_flag == 1)){
        ...
        bcov_out <- filter(bcov_targets, bcov_flag == 1)
        ### print those out
        message(bcov_out)
        ...
        bcov_out2$bcov_flag <- 1
        
        bcov_targets <- bcov_targets %>% select(Sample, Target, bcov_flag)
        
        new_file_in <- merge(new_file_in, bcov_targets, by = c("Sample, Target"), all.x = TRUE)
        
      } else {
        new_file_in$bcov_flag <- 0
