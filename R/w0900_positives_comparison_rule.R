...
#' called 'sample_pos_limit_flag' that will have a value of 1 if the row violated 
...
#' @return A dataframe just like the input data frame, with one new column (sample_pos_limit_flag) added
#' @export
...
w0900_positives_comparison_rule <- function(new_file_in, pos_samp_str, target_str, control_ids, thresh_size = 3){
  ...
  if (nrow(pos_to_flag) > 0){
    ### add pos flag
    pos_to_flag$sample_pos_limit_flag <- 1
    new_file_in <- merge(new_file_in, pos_to_flag, by = c("Sample", "Target", "Positives"), all.x = TRUE, all.y = FALSE)
    new_file_in <- new_file_in %>% mutate(sample_pos_limit_flag = case_when(is.na(sample_pos_limit_flag) ~ 0, 
                                                                            T ~ sample_pos_limit_flag))
    
  } else {
    
    new_file_in$sample_pos_limit_flag <- 0
